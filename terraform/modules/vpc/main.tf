...
# Existing private subnets (keep for RDS)
...
# New EKS-dedicated subnets with larger CIDR blocks
resource "aws_subnet" "private_eks" {
  ...
  tags = {
    Name = "${var.cluster_name}-private-eks-${count.index + 1}"
...
# Karpenter nodes egress rules
resource "aws_security_group_rule" "karpenter_nodes_egress_https" {
  type              = "egress"
  from_port         = 443
  to_port           = 443
  protocol          = "tcp"
  cidr_blocks       = ["0.0.0.0/0"]
  security_group_id = aws_security_group.cluster.id
  description       = "Allow HTTPS traffic to internet (for EKS API access)"
}

resource "aws_security_group_rule" "karpenter_nodes_egress_http" {
  type              = "egress"
  from_port         = 80
  to_port           = 80
  protocol          = "tcp"
  cidr_blocks       = ["0.0.0.0/0"]
  security_group_id = aws_security_group.cluster.id
  description       = "Allow HTTP traffic to internet (for package downloads)"
}